...
In <tt>test/fixie directory</tt>, create an <tt>*.rb</tt> file to populate the database:
...
Records created in this way are returned for use later in the population script:
    ...
    sam = Person.fixie(:sam, :name => "Sam", :email => "sam@example.com")
    sam.roles << admin

Invalid records will raise an exception and your tests will not run.

You may have as may fixture files as you like so organize the test data however it makes sense.
...
run. This is accomplished by appending to the +db:test:prepare+ task and
inserting the records into the test database by loading the fixture
files.
...
This is a work in progress. I am currently using it as I feel it's
better than the ad hoc scripts I've been using it, but I am eager to
hear your suggestions on how to improve this library.
...
* create <tt>test/fixie</tt> if it doesn't exist when the plugin is installed
